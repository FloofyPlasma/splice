...
namespace splice::detail
{
  ...
  consteval bool is_serializable_struct()
  {
    ...
    return member_count<StructType,
               [](std::meta::info member) { return is_serializable_field(serializable_by_default, member); }>()
           > 0;
  }
  ...
  SerializationTarget::IntermediaryType encode_struct(StructType object)
  {
    ...
    template for (constexpr auto member: [:std::meta::reflect_constant_array(std::meta::nonstatic_data_members_of(
                                               ^^StructType, std::meta::access_context::current())):])
    {
      ...
      if (is_serializable_field(serializable_by_default, member))
      {
        ...
        std::string key = field_annotation.has_value() && field_annotation.value().name != nullptr
                              ? static_cast<std::string>(field_annotation.value().name)
                              : static_cast<std::string>(std::meta::identifier_of(member));
